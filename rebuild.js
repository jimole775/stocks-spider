...
const { readFile, writeFile } = require('./src/utils/index')
...
;(() => {
  ...
  files.forEach(async(file, index) => {
    const data = await readFile(path.join(srcDir, file))
    const stock = file.split('.').shift()
    const date = moment(data.date).format('YYYY-MM-DD')
    await writeFile(path.join(targetDir, `${stock}_${date}.json`), JSON.stringify(data))
  })
})()