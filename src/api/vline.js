...
module.exports = function kline (req, res) {
  ...
  try {
    const { pageNumber, pageSize, date: queryDate, code: queryCode, dateRange: queryDateRange } = req.body
    const start = (Number.parseInt(pageNumber) - 1) * Number.parseInt(pageSize)
    const dates = readDirSync(global.db)
    let loop = 0
    dates.forEach((date) => {
      // 匹配 date 查询，如果 queryDate 有值，但是匹配不到对应的date，直接退出
      if (queryDate && queryDate !== date) return false
      const codes = readDirSync(path.join(global.db, date))
      codes.forEach((code) => {
        ...
        if (loop > start && loop < (start + pageSize)) {
          const item = readFileSync(path.join(global.db, date, code))
          item.date = date
